...
mod objects;
pub use crate::objects::{Light, Sphere};
...
impl RayTracer {
    pub fn draw_image(&self, image: Clamped<&[u8]>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let new_data =
            web_sys::ImageData::new_with_u8_clamped_array_and_sh(image, self.width, self.height)
                .map_err(|_| "New data failed")?;

        context
            .put_image_data(&new_data, 0., 0.)
            .map_err(|_| "Image put failed")?;
    ...
    fn calculate_light(
        ...
        let mut result = (AMBIENT_LIGHT.0, AMBIENT_LIGHT.1, AMBIENT_LIGHT.2);
        ...
        for light in self.scene.1.iter() {
            ...
            let strength = (normal.dot(light_direction)
                / (normal.length() * light_direction.length()))
            .clamp(0., 1.);
            ...
            let specular = (reflection_vector.dot(camera_vector)
                / (reflection_vector.length() * camera_vector.length()))
            .clamp(0., 1.)
    ...
    fn trace_ray(&self, ray: &Ray) -> Option<(Sphere, Vec3)> {
    ...
    fn render_pixel(
        ...
        match self.trace_ray(&ray) {
            Some((object, hit_point)) => {
                ...
                let brightness =
                    self.calculate_light(hit_point, normal, camera.from, object.material);
                (
    ...
    pub fn rs_render(&self) -> Result<(), JsValue> {
        ...
        self.draw_image(image).map_err(|e| e.to_string())?;
